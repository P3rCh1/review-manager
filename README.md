...
example:
    ...
    old_reviewer_id: u2
```
**В `docs/openapi.yaml` исправленный пример*
...
* При смене команды или выставлении флага `is_active` = `false` продолжает быть ревьюером там, где назначен (Предполагаю, что при требовании к перераспределению необходимо было как-то вернуть новых ревьюеров)
