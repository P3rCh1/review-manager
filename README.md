...
* При смене команды или выставлении флага `is_active` = `false` пользователь продолжает быть ревьюером там, где назначен (предполагаю, что при требовании к перераспределению необходимо было бы как-то вернуть новых ревьюеров)
